...
This document provides a comprehensive reference for all native tools available in the Claude Code CLI (v2.1.37). Each tool listing includes its parameters, types, behavioral characteristics, usage examples, and common patterns.

## Table of Contents

- [Claude Code Native Tools Reference](#claude-code-native-tools-reference)
  - [Table of Contents](#table-of-contents)
  - [Glossary](#glossary)
  - [Quick Reference](#quick-reference)
    - [Essential Tools (Most Common)](#essential-tools-most-common)
    - [By Use Case](#by-use-case)
    - [Full Tool Matrix](#full-tool-matrix)
  ...
  - [Common Workflow Patterns](#common-workflow-patterns)
    - [Pattern 1: Read → Edit (Most Common)](#pattern-1-read--edit-most-common)
    - [Pattern 2: Glob → Grep → Read (Codebase Exploration)](#pattern-2-glob--grep--read-codebase-exploration)
    - [Pattern 3: Background Task Execution](#pattern-3-background-task-execution)
    - [Pattern 4: Parallel Independent Operations](#pattern-4-parallel-independent-operations)
    - [Pattern 5: Sub-Agent Delegation](#pattern-5-sub-agent-delegation)
  - [Permission \& Security Model](#permission--security-model)
    - [Auto-Allowed Tools (No User Prompt)](#auto-allowed-tools-no-user-prompt)
    - [Permission-Required Tools](#permission-required-tools)
    - [Sandbox Model (Bash)](#sandbox-model-bash)
  ...
  - [Configuration Reference](#configuration-reference)
    ...
    - [Hook Enhancements](#hook-enhancements)
    - [Keybindings](#keybindings)
    - [Skill System Enhancements](#skill-system-enhancements)
  - [Summary](#summary)
...
| Track progress | `TaskCreate` / `TaskUpdate` | `TodoWrite` (legacy) |
...
- **Concurrency-safe:** Yes
- **Requires permission:** No
- **Max result size:** 100,000 characters (result list may be truncated for very large matches)
...
- **Availability:** Platform-dependent — always available on firstParty; on Vertex, requires claude-opus-4-6/sonnet-4-5/haiku-4-5 models.
- **Max result size:** 100,000 characters
...
- Setting status to `"deleted"` permanently removes a task (added in v2.1.20).
- Always use `TaskGet` to read a task's latest state before updating to avoid stale writes.
...
| `CLAUDE_CODE_ENABLE_TASKS` | `true` | TaskCreate/TaskUpdate/TaskList/TaskGet | Set `false` to revert to the legacy TodoWrite system |
| `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS` | — | Bash, Task | Disable background task functionality |
| `CLAUDE_CODE_TMPDIR` | — | All | Override temp directory location |
| `CLAUDE_CODE_FILE_READ_MAX_OUTPUT_TOKENS` | — | Read | Override file read token limit |
| `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD` | — | All | Load `CLAUDE.md` from additional directories |
| `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS` | — | All | Avoid beta header validation errors |
...
| `--tools <tools>` | Restrict built-in tools available in interactive sessions |
| `--from-pr <number\|url>` | Resume a session linked to a GitHub PR |
| `--agent <name>` | Override agent setting for current session |
| `--init` / `--init-only` / `--maintenance` | Trigger the Setup hook lifecycle event |
...
Notable built-in slash commands available in interactive sessions:

| Command | Description |
|---------|-------------|
| `/sandbox` | Open a sandboxed shell environment |
| `/keybindings` | View and customize keyboard shortcuts |
| `/teleport` | Transfer context to a remote Claude.ai session |
| `/remote-env` | Configure remote environment settings |
| `/plan` | Enter plan mode for complex task design |
| `/config` | View or modify runtime configuration |
| `/stats` | Display session statistics (tokens, tool calls) |
| `/doctor` | Diagnose configuration and connectivity issues |

### Hook Enhancements

Hooks support several advanced features beyond basic stdin JSON parsing:

- **`additionalContext` return parameter** (v2.1.9): PreToolUse hooks can return an `additionalContext` string that is injected into the tool call context, allowing hooks to annotate tool inputs with extra information.
- **`updatedInput` in `ask` permission decisions** (v2.1.9): When a PreToolUse hook returns `"decision": "ask"`, it can include an `updatedInput` object to modify the tool's input parameters before prompting the user.
- **Setup hook event** (v2.1.0): A `Setup` event fires on session initialization (and when triggered via `--init` / `--init-only` / `--maintenance` flags), enabling hooks to run setup logic before the first prompt.
- **`TeammateIdle` and `TaskCompleted` hook events** (v2.1.33): New hook events for multi-agent workflows. `TeammateIdle` fires when a teammate agent becomes idle; `TaskCompleted` fires when a spawned task finishes.
- **`once: true` configuration**: Set `once: true` on a hook definition to ensure it runs only once per session, regardless of how many times the triggering event occurs.

### Keybindings

Claude Code supports customizable keyboard shortcuts (v2.1.18). Use the `/keybindings` command to view current bindings and customize them. Keybinding configuration is stored in `~/.claude/keybindings.json`.

### Skill System Enhancements

The skill system supports several advanced features beyond the basic `SKILL.md` directory convention:

- **Hot-reload:** Skills are loaded from both `~/.claude/skills` (user-level) and `.claude/skills` (project-level) and are reloaded automatically when files change.
- **`context: fork` execution:** Skills can specify `context: fork` in their frontmatter to run in a forked execution context, isolating their tool calls from the main conversation.
- **Hook support in frontmatter:** Skill `SKILL.md` files can define hooks in their frontmatter that activate only when that skill is running.
- **`memory` frontmatter field** (v2.1.33): Agents can declare persistent memory with `user`, `project`, or `local` scope, enabling cross-session state without manual memory management.
- **`Task(agent_type)` tool restriction** (v2.1.33): Agent `tools` frontmatter can restrict which sub-agent types are spawnable using `Task(agent_type)` syntax (e.g., `Task(Bash)`, `Task(Explore)`).
- **`${CLAUDE_SESSION_ID}` substitution:** Skill definitions support `${CLAUDE_SESSION_ID}` variable substitution in their configuration, useful for session-scoped temp files or logging.
...
All tool definitions extracted from the installed Claude Code package (`@anthropic-ai/claude-code` v2.1.37). Schemas are validated at runtime using [Zod](https://zod.dev).
